...
b = [1,2,1]
#直接赋值，指向统一内存地址，共用一份数据，任何一方更改，会影响另一个
a = b
print(id(a))
print(id(b))
...
#copy 只拷贝一层，如果copy的是引用，则是引用，数据修改也会源数跟着改变，如果是copy的是数据则是新存自己一份数据。
#例如：m列表保存了j,k的引用，l用copy.copy(m)则是新建内存地址，拷贝m一次。发现m里面是引用的j,k的内存数据，所以l里面也存着指向j,k的内存数据, 当源数据j添加一个4，则l也会跟着改变添加了一个4。
#     s是j的一层拷贝，发现j里面直接就是数据了，所有会新存自己的一份数据，当j改变时，q不会改变了
j = [1,2,3]
k = [7,8,9]
m = [j,k]
